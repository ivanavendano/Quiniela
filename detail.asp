...
Las quinielas marcadas en rojo están identificadas como <font color="#FF0000" size="3"><b>traidores a la patria</b></font> por colocar perdiendo a La Vinotinto en algún partido
<br><br>
  </center></td></tr>
  <tr>
...
<%
		  ...
		  sql = "SELECT t1.short as ts1, t2.short as ts2, gg.goalsteam1 as g1, gg.goalsteam2 as g2, gg.updated, gg.idgame FROM betformats_games bg, groupgames gg, teams t1, teams t2 ,roundrangedates rrd WHERE bg.idgame=gg.idgame AND gg.idteam1 = t1.idteam AND gg.idteam2 = t2.idteam AND bg.idbetformat = " & betformatID & " AND gg.display=1 and gg.idronda in (1,2,3) AND gg.idronda=rrd.idronda AND DATE()-rrd.fechainicio >= 0  ORDER BY gg.localdate+(gg.venezuelandate+1+60/60)/24, gg.idgame"
...
<%		   
		  ...
		  Do while not rsBets.EOF
		      sql = "SELECT count(*) as cnt FROM users_bets_results ubr WHERE ubr.idbet = " & rsBets.Fields("idbet") & " AND ((idteam1 = 3 and ubr.goalsteam1 < ubr.goalsteam2) OR (idteam2 = 3 and ubr.goalsteam2 < ubr.goalsteam1))"
...
<%
  		    sql = "SELECT ubr.goalsteam1 as g1, ubr.goalsteam2 as g2, ubr.points, gg.idgame FROM users_bets_results ubr, groupgames gg,roundrangedates rrd WHERE ubr.idgame = gg.idgame AND idbet = " & rsBets.Fields("idbet") & " AND GG.IDRONDA in (1,2,3) AND gg.idronda=rrd.idronda AND DATE()-rrd.fechainicio >= 0  ORDER BY gg.localdate+(gg.venezuelandate+1+60/60)/24, gg.idgame"
